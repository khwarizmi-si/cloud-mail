<script lang="ts">
	...
	function openMessage(message: Mail) {
		sessionStorage.setItem('mail:message', JSON.stringify(message));
		goto(`/inbox/${message.emailId}`);
	}
...
<main>
	...
	<section>
		...
		{:else}<ul>{#each messages as message}<li><button class="mail-row" onclick={() => openMessage(message)}>
...
<style>
	...
	ul { margin: 0; padding: 0; list-style: none; } li { border-top: 1px solid #edf1ef; } .mail-row { display: flex; width: 100%; align-items: start; justify-content: space-between; gap: 16px; padding: 16px; border: 0; background: transparent; color: inherit; text-align: left; cursor: pointer; } .mail-row:hover { background: #f5f7f4; } li div { display: grid; gap: 4px; min-width: 0; } li span, small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } small, time { color: #60756e; } time { white-space: nowrap; font-size: .8rem; }
